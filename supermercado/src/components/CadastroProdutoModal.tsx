import React from 'react';
import { Modal, TextInput, NumberInput, Textarea, Button, Group, Stack } from '@mantine/core';
import { useForm } from '@mantine/form';
import { notifications } from '@mantine/notifications';
import { IconPackage, IconCurrencyDollar, IconFileText, IconHash, IconPlus } from '@tabler/icons-react';
import type { Produto } from '../schemas/produto';

interface CadastroProdutoModalProps {
  opened: boolean;
  onClose: () => void;
  onSuccess: (produto: Produto) => void;
}

export const CadastroProdutoModal: React.FC<CadastroProdutoModalProps> = ({
  opened,
  onClose,
  onSuccess,
}) => {
  const form = useForm({
    initialValues: {
      nome: '',
      preco: 0,
      descricao: '',
      quantidade: 0,
    },

    validateInputOnBlur: true,

    validate: {
      nome: (value) => {
        if (!value.trim()) return 'O nome do produto é obrigatório';
        if (value.trim().length < 2) return 'O nome deve ter pelo menos 2 caracteres';
        return null;
      },
      preco: (value) => {
        if (value === undefined || value === null || typeof value !== 'number' || isNaN(value)) {
          return 'O preço é obrigatório';
        }
        if (value <= 0) return 'O preço deve ser maior que zero';
        return null;
      },
      descricao: (value) => {
        if (!value.trim()) return 'A descrição é obrigatória';
        if (value.trim().length < 3) return 'A descrição deve ter pelo menos 3 caracteres';
        return null;
      },
      quantidade: (value) => {
        if (value === undefined || value === null || typeof value !== 'number' || isNaN(value)) {
          return 'A quantidade é obrigatória';
        }
        if (value < 0) return 'A quantidade não pode ser negativa';
        return null;
      },
    },
  });

  const handleSubmit = (values: typeof form.values) => {
    const novoProduto: Produto = {
      _id: Math.random().toString(36).substring(2, 18),
      nome: values.nome,
      preco: Number(values.preco),
      descricao: values.descricao,
      quantidade: Number(values.quantidade),
    };

    onSuccess(novoProduto);

    notifications.show({
      title: 'Produto cadastrado',
      message: `O produto "${values.nome}" foi cadastrado com sucesso!`,
      color: 'green',
    });

    form.reset();
    onClose();
  };

  const handleClose = () => {
    form.reset();
    onClose();
  };

  return (
    <Modal
      opened={opened}
      onClose={handleClose}
      title="Cadastrar Novo Produto"
      centered
      size="md"
    >
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Stack>
          <TextInput
            label="Nome do Produto"
            placeholder="Ex: Teclado Mecânico"
            leftSection={<IconPackage size={18} />}
            {...form.getInputProps('nome')}
          />

          <NumberInput
            label="Preço (R$)"
            placeholder="0.00"
            decimalScale={2}
            fixedDecimalScale
            min={0}
            leftSection={<IconCurrencyDollar size={18} />}
            {...form.getInputProps('preco')}
          />

          <Textarea
            label="Descrição"
            placeholder="Descrição detalhada do produto"
            rows={3}
            leftSection={<IconFileText size={18} />}
            {...form.getInputProps('descricao')}
          />

          <NumberInput
            label="Quantidade em Estoque"
            placeholder="0"
            min={0}
            allowDecimal={false}
            leftSection={<IconHash size={18} />}
            {...form.getInputProps('quantidade')}
          />

          <Group justify="flex-end" mt="md">
            <Button variant="outline" onClick={handleClose}>
              Cancelar
            </Button>
            <Button type="submit" variant="filled" leftSection={<IconPlus size={18} />}>
              Cadastrar Produto
            </Button>
          </Group>
        </Stack>
      </form>
    </Modal>
  );
};
